...
# Load KMeans model from MLflow
@st.cache_resource
def load_kmeans_model():
    with open("clustering_models/model_metrics.json", "r") as f:
        metrics = json.load(f)

    mlruns_path = os.path.abspath(os.path.join("clustering_models", "mlruns"))
    mlflow.set_tracking_uri(f"file:///{mlruns_path.replace(os.sep, '/')}")  # Use your actual run_id here
    model = mlflow.sklearn.load_model(
        f"runs:/{metrics['models']['clustering']['KMeans']['run_id']}/KMeans_Model"
    )
    return model
...
page = st.sidebar.radio("Select a Page", ["Crime Heatmap", "KMeans Clustering Image", "Predict Cluster"])
...
if page == "Crime Heatmap":
    ...
    if 'KMeans_Cluster' not in df.columns:
        ...
        kmeans.cluster_centers_ = np.array([df[df['KMeans_Cluster'] == i][['Latitude', 'Longitude']].mean().values for i in range(7)])
    ...
    # Predict cluster page
    st.header("Predict Crime Cluster for Given Coordinates")
    ...
    # Load the pre-trained KMeans model from MLflow (cached)
    model = load_kmeans_model()

    # Make prediction based on the input coordinates
    # if latitude != 0.0 and longitude != 0.0:
        # Predict the cluster for the entered coordinates
    prediction = model.predict(np.array([[latitude, longitude]]))
    st.write(f"The predicted cluster for the given coordinates is: {prediction[0]}")
    # else:
    #     st.write("Please enter valid latitude and longitude values.")